...
class Teeko2Player:
    ...
    def heuristic_game_value(self, state, piece): #This method is designed to check for the largest connected amount of pieces
        ...
        for row in range(3, 5):
            for i in range(2):
                if state[row][i] == my_piece:
                    mycnt += 1
                if state[row - 1][i + 1] == my_piece:
                    mycnt += 1
                if state[row - 2][i + 2] == my_piece:
                    mycnt += 1
                if state[row - 3][i + 3] == my_piece:
                    mycnt += 1
        ...
        for row in range(3, 5):
            for i in range(2):
                if state[row][i] == ai:
                    oppcnt += 1
                if state[row - 1][i + 1] == ai:
                    oppcnt += 1
                if state[row - 2][i + 2] == ai:
                    oppcnt += 1
                if state[row - 3][i + 3] == ai:
                    oppcnt += 1
                if oppcnt > oppmax:
                    oppmax = oppcnt
                oppcnt = 0
        ...
        for row in range(3):
            ...
            for i in range(3):
                if state[row][col] == my_piece:
                    mycnt += 1
                if state[row+1][col-1] == my_piece:
                    mycnt += 1
                if state[row+1][col+1] == my_piece:
                    mycnt += 1
                if state[row+2][col]== my_piece:
                    mycnt += 1
                if mycnt > mymax:
                    mymax = mycnt
                mycnt = 0
        ...
        for row in range(3):
            ...
            for i in range(3):
                if state[row][col] == ai:
                    oppcnt += 1
                if state[row+1][col-1] == ai:
                    oppcnt += 1
                if state[row+1][col+1] == ai:
                    oppcnt += 1
                if state[row+2][col]== ai:
                    oppcnt += 1
                if oppcnt > oppmax:
                    oppmax = oppcnt
                oppcnt = 0
    ...
    def game_value(self, state):
        ...
        for row in range(3):
            col = 1
            for i in range(3):
                if state[row][col] != ' ' and state[row][col] == state[row+1][col-1] == state[row+1][col+1] == state[row+2][col]:
